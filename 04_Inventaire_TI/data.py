...
class User:
	def __init__(	self, name: str, id = 0 ) -> None:
...
class Product:
	def __init__(	self, name: str, value: int, user: User = None ) -> None:
		self.__name: str = name
...
class Screen ( Product ):
	...
	def checkScreenAttributes(
		self,
		size: float = None,
		hdmi: bool = None
	) -> bool:
		if (
			( size and round(size, 2) != self.__size ) or
...
class Keyboard ( Product ):
	...
	def checkKeyboardAttributes(
		...
		if (
			( wireless and wireless != self.__wireless ) or
...
class Inventory:
	...
	"Mouse": [Mouse, 0]
		...
		for product in self.__stock:
			...
			total += current_value
		for type in types:
			print( f"Value of {type.lower()}s in inventory {ASSOC_SYM} " +
	f"{types[type][1]} $" )
